...
class TestTickerLister(unittest.TestCase):
    ...
    def test_list_all_tickers(self):
        from processor.us_daily.ticker_lister import list_all_tickers
        ...
        # Called without exchange filter
        client.list_tickers.assert_called_once_with(
            market="stocks", active=True, limit=1000
        )

        file_path = os.path.join(self.test_dir, "tickers.json")
